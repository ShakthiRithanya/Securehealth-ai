...
export default function NurseDashboard() {
    ...
    const handleWsMessage = useCallback((msg) => {
        if (msg.event === 'patient_action') {
            if (
                ...
                !msg.patient_ward ||
                assignedWards.includes(msg.patient_ward)
            ) {
                if (msg.user_role === 'nurse' && msg.user_id !== user.id) {
                    return
                }
                setActivityFeed((prev) => [msg, ...prev].slice(0, 50))
            }
        }
    }, [assignedWards, user.id])
    ...
    return (
        <div className="min-h-screen bg-transparent relative">
            <Navbar />
            <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col gap-8 animate-fade-in z-10 relative">
                { }
                <div className="flex items-start justify-between flex-wrap gap-3">
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-3">
                            ...
                            {user?.supervising_doctor_name && (
                                <span className="bg-indigo-500/15 text-indigo-400 border border-indigo-500/25 text-xs font-semibold px-3 py-1 rounded-full">
                                    👨‍⚕️ {user.supervising_doctor_name}
                                </span>
                            )}
